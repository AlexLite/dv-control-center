...
class instance extends instance_skel {
	...
	constructor(system, id, config) {
		...
		this.auto_port;
		this.dvccMergePollTimer = null;
		this.dvccBaseUrl = 'http://127.0.0.1:9999';
		this.dvcc_merge_active_flex = '';
		this.dvcc_merge_running_flex = '';
		this.dvcc_merge_active_pip = '';
		this.dvcc_merge_running_pip = '';
	...
	action(action) {
		...
		switch (id) {
			case 'switch_pgm':
				if (this.isSE3200VirtualFlexOption(options.switchpgm)) {
					cmd = this.buildSE3200VirtualFlexSwitchCommands('pgm', options.switchpgm);
				} else {
				...
				break;
			case 'switch_pvw':
				if (this.isSE3200VirtualFlexOption(options.switchpvw)) {
					cmd = this.buildSE3200VirtualFlexSwitchCommands('pvw', options.switchpvw);
				} else {
			...
			case 'audio_src':
				...
				break;
			case 'se3200_multiview':
				if (this.config.modelID == 'se3200' && !this.model.legacy_dvip) {
					const mvUpdates = [];
					const mvLabels = [
						{ label: 'OUTPUT_MULTIVIEWER_MODE', value: options.mode },
						{ label: 'OUTPUT_MULTIVIEWER_MAIN1_SRC', value: options.main1 },
						{ label: 'OUTPUT_MULTIVIEWER_MAIN2_SRC', value: options.main2 },
						{ label: 'OUTPUT_MULTIVIEWER_TRANSP_LABELS', value: options.transparent_labels },
						{ label: 'OUTPUT_MULTIVIEWER_AUTO_NUM', value: options.auto_num },
						{ label: 'OUTPUT_MULTIVIEWER_LABEL_INFO', value: options.label_info },
					];

					for (let i = 0; i < mvLabels.length; i++) {
						const control = this.getControlMetaByLabel(mvLabels[i].label);
						if (control !== undefined) {
							mvUpdates.push({ ...control, value: mvLabels[i].value });
						}
					}
					if (mvUpdates.length > 0) {
						cmd = this.buildSetControlCommand(mvUpdates);
					}
				}
				break;
			case 'se3200_flex_sources':
				if (this.config.modelID == 'se3200' && !this.model.legacy_dvip) {
					const flexUpdates = [];
					const flexLabels = [
						{ label: 'SWITCHER_FLEX_SRC_BGND_SRC', value: options.bg },
						{ label: 'SWITCHER_FLEX_SRC_DVE1_SRC', value: options.dve1 },
						{ label: 'SWITCHER_FLEX_SRC_DVE2_SRC', value: options.dve2 },
						{ label: 'SWITCHER_FLEX_SRC_DVE3_SRC', value: options.dve3 },
						{ label: 'SWITCHER_FLEX_SRC_DVE4_SRC', value: options.dve4 },
						{ label: 'SWITCHER_FLEX_SRC_FGND_SRC', value: options.fg },
						{ label: 'SWITCHER_FLEX_SRC_FGND_ENABLE', value: options.fg_enable },
					];

					for (let i = 0; i < flexLabels.length; i++) {
						const control = this.getControlMetaByLabel(flexLabels[i].label);
						if (control !== undefined) {
							flexUpdates.push({ ...control, value: flexLabels[i].value });
						}
					}
					if (flexUpdates.length > 0) {
						cmd = this.buildSetControlCommand(flexUpdates);
					}
				}
				break;
			case 'se3200_flex1_run':
				if (this.config.modelID == 'se3200' && !this.model.legacy_dvip) {
					const flex1Updates = [];
					const flex1Labels = [
						{ label: 'SWITCHER_FLEX_SRC_DVE1_SRC', value: options.dve1 },
						{ label: 'SWITCHER_FLEX_SRC_FGND_SRC', value: options.fg },
						{ label: 'SWITCHER_FLEX_SRC_FGND_ENABLE', value: 1 },
					];
					for (let i = 0; i < flex1Labels.length; i++) {
						const control = this.getControlMetaByLabel(flex1Labels[i].label);
						if (control !== undefined) {
							flex1Updates.push({ ...control, value: flex1Labels[i].value });
						}
					}

					//Flex sub-section uses PINP control IDs with sub-section = 2
					flex1Updates.push({ sectionID: 2, subSectionID: 2, controlID: 0, type: 'flag', value: 1 }); // ENABLE
					flex1Updates.push({ sectionID: 2, subSectionID: 2, controlID: 1, type: 'float', value: options.pos_x }); // POSITION_X
					flex1Updates.push({ sectionID: 2, subSectionID: 2, controlID: 2, type: 'float', value: options.pos_y }); // POSITION_Y
					flex1Updates.push({ sectionID: 2, subSectionID: 2, controlID: 5, type: 'float', value: options.size_x }); // SIZE_X
					flex1Updates.push({ sectionID: 2, subSectionID: 2, controlID: 6, type: 'float', value: options.size_y }); // SIZE_Y

					if (flex1Updates.length > 0) {
						cmd = this.buildSetControlCommand(flex1Updates);
					}
				}
				break;
			case 'se3200_flex2_run':
				if (this.config.modelID == 'se3200' && !this.model.legacy_dvip) {
					const flex2Updates = [];
					const flex2Labels = [
						{ label: 'SWITCHER_FLEX_SRC_DVE2_SRC', value: options.dve2 },
					...
					for (let i = 0; i < flex2Labels.length; i++) {
						const control = this.getControlMetaByLabel(flex2Labels[i].label);
						if (control !== undefined) {
							flex2Updates.push({ ...control, value: flex2Labels[i].value });
						}
					}

					//Flex sub-section uses PINP control IDs with sub-section = 2
					flex2Updates.push({ sectionID: 2, subSectionID: 2, controlID: 37, type: 'flag', value: 1 }); // ENABLE
					flex2Updates.push({ sectionID: 2, subSectionID: 2, controlID: 38, type: 'float', value: options.pos_x }); // POSITION_X
					flex2Updates.push({ sectionID: 2, subSectionID: 2, controlID: 39, type: 'float', value: options.pos_y }); // POSITION_Y
					flex2Updates.push({ sectionID: 2, subSectionID: 2, controlID: 42, type: 'float', value: options.size_x }); // SIZE_X
					flex2Updates.push({ sectionID: 2, subSectionID: 2, controlID: 43, type: 'float', value: options.size_y }); // SIZE_Y

					if (flex2Updates.length > 0) {
						cmd = this.buildSetControlCommand(flex2Updates);
					}
				}
				break;
			case 'se3200_flex_stop':
				if (this.config.modelID == 'se3200' && !this.model.legacy_dvip) {
					const stopUpdates = [];
					const flexStopControl = this.getControlMetaByLabel('SWITCHER_FLEX_SRC_FGND_ENABLE');
					if (flexStopControl !== undefined) {
						stopUpdates.push({ ...flexStopControl, value: 0 });
					}
					//Disable all four Flex PINP windows in sub-section 2
					stopUpdates.push({ sectionID: 2, subSectionID: 2, controlID: 0, type: 'flag', value: 0 });
					stopUpdates.push({ sectionID: 2, subSectionID: 2, controlID: 37, type: 'flag', value: 0 });
					stopUpdates.push({ sectionID: 2, subSectionID: 2, controlID: 74, type: 'flag', value: 0 });
					stopUpdates.push({ sectionID: 2, subSectionID: 2, controlID: 111, type: 'flag', value: 0 });

					cmd = this.buildSetControlCommand(stopUpdates);
				}
				break;
			case 'se3200_pip_window':
				if (this.config.modelID == 'se3200' && !this.model.legacy_dvip) {
					const window = parseInt(options.window);
					if (!Number.isNaN(window) && window >= 1 && window <= 4) {
						const pipUpdates = [];
						const pipLabels = [
							{ label: `SWITCHER_PINP${window}_ENABLE`, value: options.enable },
							{ label: `SWITCHER_PINP${window}_POSITION_X`, value: options.pos_x },
							{ label: `SWITCHER_PINP${window}_POSITION_Y`, value: options.pos_y },
							{ label: `SWITCHER_PINP${window}_SIZE_X`, value: options.size_x },
							{ label: `SWITCHER_PINP${window}_SIZE_Y`, value: options.size_y },
							{ label: `SWITCHER_PINP${window}_EDGE_LEFT`, value: options.edge_left },
							{ label: `SWITCHER_PINP${window}_EDGE_RIGHT`, value: options.edge_right },
							{ label: `SWITCHER_PINP${window}_EDGE_TOP`, value: options.edge_top },
							{ label: `SWITCHER_PINP${window}_EDGE_BOTTOM`, value: options.edge_bottom },
						];

						for (let i = 0; i < pipLabels.length; i++) {
							const control = this.getControlMetaByLabel(pipLabels[i].label);
							if (control !== undefined) {
								pipUpdates.push({ ...control, value: pipLabels[i].value });
							}
						}
						if (pipUpdates.length > 0) {
							cmd = this.buildSetControlCommand(pipUpdates);
						}
					}
				}
				break;
			case 'se3200_set_control_label':
				if (this.config.modelID == 'se3200' && !this.model.legacy_dvip) {
					const label = options.label ? options.label.toString().trim() : '';
					const control = this.getControlMetaByLabel(label);
					if (control !== undefined) {
						let valueType = options.value_type;
						if (valueType == 'auto') {
							valueType = control.type;
						}
						cmd = this.buildSetControlCommand([{
							sectionID: control.sectionID,
							subSectionID: control.subSectionID,
							controlID: control.controlID,
							type: valueType,
							value: options.value,
						}]);
					} else {
						this.log('warn', `SE-3200 control label not found: ${label}`);
					}
				}
				break;
			case 'se3200_set_control_raw':
				if (this.config.modelID == 'se3200' && !this.model.legacy_dvip) {
					cmd = this.buildSetControlCommand([{
						sectionID: parseInt(options.section_id),
						subSectionID: parseInt(options.sub_section_id),
						controlID: parseInt(options.control_id),
			...
			case 'dvcc_merge_run_named':
				this.runDvccMergeKeyByName(options);
				return;
			case 'send_hex':
			...
			if (this.socket !== undefined && this.socket.connected) {
				if (!this.model.legacy_dvip) {
					...
					for (let i = 0; i < cmdList.length; i++) {
						this.socket.send(this.prependPacketSize(cmdList[i]));
					}
				...
				this.consoleLog("Send (" + id + "): ", cmdList.length === 1 ? cmdList[0] : cmdList);
			} else {
	...
	resolveDvccText(input, fallback = '') {
		const source = (input === undefined || input === null || input === '') ? fallback : input;
		return String(source == null ? '' : source);
	}

	resolveDvccBaseUrl(options = null) {
		const configured = options && options.base_url ? options.base_url : '';
		const fallback = this.dvccBaseUrl || 'http://127.0.0.1:9999';
		const resolved = this.resolveDvccText(configured, fallback).trim();
		if (resolved) this.dvccBaseUrl = resolved;
		return this.dvccBaseUrl || 'http://127.0.0.1:9999';
	}

	async dvccRequestJson(urlObj, method = 'GET') {
	...
	setDvccMergeState(mode, payload) {
		const kind = String(mode || 'flex').toLowerCase() === 'pip' ? 'pip' : 'flex';
		const active = String(payload && payload.activePresetName ? payload.activePresetName : '').trim();
		const running = String(payload && payload.runningPresetName ? payload.runningPresetName : '').trim();
		if (kind === 'pip') {
			this.dvcc_merge_active_pip = active;
			this.dvcc_merge_running_pip = running;
			this.setVariable('dvcc_merge_active_pip', active);
			this.setVariable('dvcc_merge_running_pip', running);
		} else {
			this.dvcc_merge_active_flex = active;
			this.dvcc_merge_running_flex = running;
			this.setVariable('dvcc_merge_active_flex', active);
			this.setVariable('dvcc_merge_running_flex', running);
		}
		this.checkFeedbacks('dvcc_merge_active');
	}

	async pollDvccMergeStateOnce() {
		const baseUrl = this.resolveDvccBaseUrl();
		for (const mode of ['flex', 'pip']) {
			try {
				const url = new URL('/api/merge/state?mode=' + mode, baseUrl);
				const payload = await this.dvccRequestJson(url, 'GET');
				this.setDvccMergeState(mode, payload || {});
			} catch (_) {
				// Keep last known state if DVCC state endpoint is unavailable.
			}
		}
	}

	startDvccMergeStatePolling() {
		this.stopDvccMergeStatePolling();
		this.pollDvccMergeStateOnce().catch(() => {});
		this.dvccMergePollTimer = setInterval(() => {
			this.pollDvccMergeStateOnce().catch(() => {});
		}, 800);
	}

	stopDvccMergeStatePolling() {
		if (this.dvccMergePollTimer) {
			clearInterval(this.dvccMergePollTimer);
			this.dvccMergePollTimer = null;
		}
	}

	async runDvccMergeKeyByName(options = {}) {
		const keyNameRaw = this.resolveDvccText(options.key_name, 'Key1').trim();
		if (!keyNameRaw) {
			this.log('warn', 'DVCC merge key name is empty');
			return;
		}
		const mode = String(options.mode || 'flex').toLowerCase() === 'pip' ? 'pip' : 'flex';
		const baseUrl = this.resolveDvccBaseUrl(options);
		const method = String(options.http_method || 'POST').toUpperCase() === 'GET' ? 'GET' : 'POST';

		try {
			const encoded = encodeURIComponent(keyNameRaw);
			const url = new URL('/api/merge/run/' + encoded + '?mode=' + mode, baseUrl);
			await this.dvccRequestJson(url, method);
			this.setDvccMergeState(mode, { activePresetName: keyNameRaw, runningPresetName: '' });
			await this.pollDvccMergeStateOnce();
		} catch (e) {
			this.log('warn', 'DVCC merge run failed: ' + (e && e.message ? e.message : e));
		}
	}
	...
	init() {
		...
		this.init_feedbacks();
		this.init_variables();
		this.startDvccMergeStatePolling();
		this.initTCP();
		this.init_presets();
		this.actions();
		this.init_commands();
	...
	isSE3200VirtualFlexOption(optionId) {
		return optionId === 'dv_virtual_flex1' || optionId === 'dv_virtual_flex2';
	}

	buildSE3200VirtualFlexSwitchCommands(bus, optionId) {
		if (this.model.legacy_dvip) {
			return [];
		}

		const transId = optionId === 'dv_virtual_flex2' ? '17' : '16';
		const transChoice = this.model.trans.find((item) => String(item.id) === transId);
		const busChoices = bus === 'pvw' ? this.model.pvw : this.model.pgm;
		const flexChoice = busChoices.find((item) => String(item.label).toLowerCase() === 'flex');
		const commands = [];

		if (transChoice !== undefined) {
			commands.push(transChoice.cmd);
		}
		if (flexChoice !== undefined) {
			commands.push(flexChoice.cmd);
		}

		return commands;
	}

	getControlMetaByLabel(label) {
		if (this.COMMANDS === undefined || this.COMMANDS[0] === undefined || this.COMMANDS[0].sections === undefined) {
			return undefined;
		}

		for (let i = 0; i < this.COMMANDS[0].sections.length; i++) {
			const section = this.COMMANDS[0].sections[i];
			if (section === undefined || section.controls === undefined) {
				continue;
			}

			const control = section.controls.find((element) => element.label === label);
			if (control !== undefined) {
				let sectionID = section.id;
				let subSectionID = 0;
				if (section.mainSection !== undefined) {
					sectionID = section.mainSection;
					subSectionID = section.subSection || 0;
				}
	...
	buildSetControlCommand(updates) {
		//Builds a DV_COMMAND_SET_CONTROL packet body from a list of control updates
		//Each update: { sectionID, subSectionID, controlID, type, value }
		let command = Buffer.from([0x01, 0x00, 0x00, 0x00]);

		for (let i = 0; i < updates.length; i++) {
			const update = updates[i];
			let controlSection = Buffer.alloc(4);
			let value = Buffer.alloc(4);
			let writeValue = Number(update.value);
			if (Number.isNaN(writeValue)) {
				writeValue = 0;
			}

			if (update.sectionID == 2) {
				controlSection.writeUInt8(update.controlID & 0xFF, 0);
				controlSection.writeUInt8((update.subSectionID || 0) & 0xFF, 1);
				controlSection.writeUInt16LE(2, 2);
			} else {
				controlSection.writeUInt16LE(update.controlID, 0);
				controlSection.writeUInt16LE(update.sectionID, 2);
			}

			switch (update.type) {
				case 'float':
					value.writeFloatLE(writeValue, 0);
					break;
				case 'flag':
					value.writeUInt8(writeValue > 0 ? 1 : 0, 0);
					break;
				case 'int':
				default:
					value.writeInt32LE(parseInt(writeValue), 0);
					break;
			}

			command = Buffer.concat([command, controlSection, value]);
		}
	...
	init_variables() {
		...
		this.setVariable('dvcc_merge_active_flex', this.dvcc_merge_active_flex || '');
		this.setVariable('dvcc_merge_running_flex', this.dvcc_merge_running_flex || '');
		this.setVariable('dvcc_merge_active_pip', this.dvcc_merge_active_pip || '');
		this.setVariable('dvcc_merge_running_pip', this.dvcc_merge_running_pip || '');
	};
