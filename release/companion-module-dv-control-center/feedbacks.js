exports.getFeedbacks = function () {
	...
	feedbacks['dvcc_merge_active'] = {
		label: 'DVCC Merge Key Active',
		description: 'Set button colors when selected DV Control Center Merge key is active.',
		options: [{
			type: 'colorpicker',
			label: 'Foreground color',
			id: 'fg',
		...
		callback: (feedback, bank) => {
			const mode = String(feedback.options.mode || 'flex').toLowerCase();
			const expected = String(feedback.options.key_name || '').trim().toLowerCase();
			if (!expected) return;
			const active = mode === 'pip'
				? String(this.dvcc_merge_active_pip || '').trim().toLowerCase()
				: String(this.dvcc_merge_active_flex || '').trim().toLowerCase();
