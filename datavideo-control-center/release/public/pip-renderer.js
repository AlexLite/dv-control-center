(function exposePipRenderer() {
  function drawPipCanvas(deps) {
    const {
      state,
      byId,
      toCanvas,
    ...
    } = deps;  normalizePipAspect();
  syncCanvasResolution('pipCanvas');
  const canvas = byId('pipCanvas');
  const ctx = canvas.getContext('2d');
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ctx.fillStyle = '#132334';
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  const density = Math.max(4, Number(state.pipGridDensity) || 24);
  ctx.strokeStyle = '#2d4252';
  ctx.lineWidth = 1;
  ...
  if (state.pipShowGuides !== false) {
    const guides = getGuidePixelsFromList(state.pipGuides, canvas.width, canvas.height);
  ...
  const colors = ['#3ec1d3', '#ff9a3c', '#f6d743', '#7dd87d'];
  const active = Number(state.pipActive) || 0;
  let visibleCount = 0;
  ...
  for (let i = 1; i <= 4; i += 1) {
    ...
    const borderOn = Number(byId(`pip${i}-bs`)?.value || 0) > 0;
    const borderPct = clamp(Number(byId(`pip${i}-bw`)?.value || 0), 0, 100) / 100;
    ...
    const cl = clamp(Number(byId(`pip${i}-cl`)?.value || 0), 0, 100) / 100;
    const cr = clamp(Number(byId(`pip${i}-cr`)?.value || 0), 0, 100) / 100;
    const ct = clamp(Number(byId(`pip${i}-ct`)?.value || 0), 0, 100) / 100;
    const cb = clamp(Number(byId(`pip${i}-cb`)?.value || 0), 0, 100) / 100;
    const innerX = r.x + (r.w * cl);
    const innerY = r.y + (r.h * ct);
    const innerW = Math.max(1, r.w * (1 - cl - cr));
    const innerH = Math.max(1, r.h * (1 - ct - cb));
    ctx.fillStyle = 'rgba(130, 138, 145, 0.24)';
    ...
    ctx.strokeStyle = 'rgba(210, 218, 224, 0.66)';
    ctx.lineWidth = 1;
    ctx.strokeRect(innerX, innerY, innerW, innerH);
    ctx.fillStyle = colors[i - 1];
    ctx.fillText(`PIP ${i}`, r.x + 6, r.y + 14);
  ...
  if (visibleCount === 0) {
    ...
    ctx.fillText('No windows enabled. Turn on checkbox for PIP 1..4.', 230, canvas.height * 0.5);
  }
}

  window.DVIPPipRenderer = {
    drawPipCanvas,
  };
})();
